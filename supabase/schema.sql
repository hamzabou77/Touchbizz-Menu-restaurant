...
CREATE TABLE IF NOT EXISTS public.qr_settings (
    ...
    style TEXT DEFAULT 'squares' NOT NULL, -- 'squares', 'dots', 'rounded'
    with_logo BOOLEAN DEFAULT true NOT NULL,
    frame_type TEXT DEFAULT 'badge' NOT NULL, -- 'none', 'badge', 'card', 'header'
...
-- 10. MENU SCAN IMPORTS TABLE (History & Stage)
CREATE TABLE IF NOT EXISTS public.menu_scan_imports (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    restaurant_id UUID NOT NULL REFERENCES public.restaurants(id) ON DELETE CASCADE,
    image_url TEXT,
    status TEXT DEFAULT 'pending' NOT NULL, -- 'pending', 'verified', 'imported'
    raw_data JSONB,
    created_at TIMESTAMPTZ DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);
...
CREATE POLICY "Languages readable if published or owner"
    ON public.restaurant_languages FOR SELECT
    USING (
        EXISTS (
...
CREATE POLICY "Owners can manage restaurant languages"
    ON public.restaurant_languages FOR ALL
    USING (
        EXISTS (
            SELECT 1 FROM public.restaurants r
            WHERE r.id = restaurant_languages.restaurant_id
...
-- 4. Categories
CREATE POLICY "Categories readable if published or owner"
...
CREATE POLICY "Category translations readable if published or owner"
...
CREATE POLICY "Owners can manage category translations"
...
-- 6. Menu Items
CREATE POLICY "Menu items readable if published or owner"
...
CREATE POLICY "Menu item translations readable if published or owner"
...
CREATE POLICY "Owners can manage menu item translations"
...
-- 8. QR Settings
CREATE POLICY "Owners can manage QR settings"
...
-- 9. Menu Scan Imports
CREATE POLICY "Owners can manage menu scan imports"
    ON public.menu_scan_imports FOR ALL
    USING (
        EXISTS (
            SELECT 1 FROM public.restaurants r
...
-- Execute these in Supabase SQL editor to create storage buckets:
-- ==============================================================================
...
-- Public can read images from public buckets
